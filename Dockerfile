...
CMD ["sh", "-c", "cd backend && python manage.py migrate --noinput && python manage.py seed_mountain_descriptions && python manage.py seed_mountain_tags && gunicorn forestrx.wsgi:application --bind 0.0.0.0:${PORT:-8000} --workers 2 --timeout 120"]
