...
11. [보호자 뷰 & 시뮬레이션](#11-보호자-뷰--시뮬레이션)
12. [AI 도우미 (RAG 채팅)](#12-ai-도우미-rag-채팅)
13. [등산로 경로 데이터](#13-등산로-경로-데이터)
14. [데이터 소스](#14-데이터-소스)
15. [배포 (Railway)](#15-배포-railway)
16. [알려진 제약사항](#16-알려진-제약사항)
17. [잔여 과제](#17-잔여-과제)
...
- **보호자 실시간 모니터링**: 30분 위치 미갱신 시 경고, 60분 시 긴급 팝업
...
4. **안전코스 찾기** → AI 브리핑 카드 최상단 + 추천 등산로 목록
5. **코스 카드 클릭** → Leaflet 지형도 + 3색 등산로 + 재난위험지구 패널 + 커뮤니티 안전 제보
6. **안전공유 탭** → "산행 시작" → **6자리 코드** 생성 (예: `A3K7PQ`)
7. **코드 공유** → 보호자가 코드 입력 → 실시간 위치 확인 (풀스크린 맵 + 하단 정보 시트)
8. **보호자 뷰 시뮬레이션** → 🎬 버튼으로 북한산 산행 체험: 이동→정지→30분 경고→60분 긴급 팝업
9. **AI 도우미 탭** → 산행 안전·코스·장비 질문 → Gemini RAG 답변
10. **산행 종료** → 기록 자동 저장 → 내정보 탭에서 확인
...
| 안전 등급 기반 등산로 목록 | ✅ | 점수 숫자 비표시, 간결한 리스트 형태 |
...
| **산 스토리 API** | ✅ | 산 선택 시 로딩 스피너 후 정보·역사 제공 |
| **산악기상 전용 API** | ✅ | 산 기준 날씨 조회 |
| 재난위험지구 매칭·감점 | ✅ | 중복 제거 + 위험유형 아이콘·색상 배지 표시 |
...
| **보호자 뷰 풀스크린 맵** | ✅ | 헤더(50px) + 맵(flex-grow) + 하단 정보 시트 |
| **보호자 30분 경고 시스템** | ✅ | 맵 위 빨간 pulsing 마커 + 배너 (실제·시뮬레이션 공통) |
| **보호자 60분 긴급 팝업** | ✅ | "위치 미갱신 60분" 모달 + 119 신고 버튼 |
| **북한산 산행 시뮬레이션** | ✅ | 도선사→하루재 5단계 이동, 30분·60분 경고 체험 |
| 커뮤니티 게시판·댓글·좋아요 | ✅ | |
| **커뮤니티 샘플 fixture 데이터** | ✅ | 5개 계정·8개 게시글·9개 댓글 Railway 적재 완료 |
| 좋아요한 게시글 목록 | ✅ | |
| 산행 기록 자동 저장 | ✅ | |
| 즐겨찾기 코스 | ✅ | |
| 긴급 연락처 관리 | ✅ | |
...
│       ├── styles.css            전역 스타일 (guardian·simulation·disaster 포함)
│       ├── views/
│       │   ├── GuideTab.vue          안전코스 추천 탭 (태그 필터·산 상세·재난위험 배지)
│       │   ├── SafeLinkTab.vue       안전공유 탭 (2컬럼)
│       │   ├── CommunityTab.vue      커뮤니티 탭 (포스트 그리드)
│       │   ├── MyPageTab.vue         내정보 탭 (3컬럼 그리드)
│       │   ├── ChatTab.vue           AI 도우미 탭 (Gemini RAG 채팅)
│       │   ├── GuardianView.vue      보호자 실시간 위치 + 북한산 시뮬레이션
...
│       │   ├── useLeafletMap.js      Leaflet + OSM 지도 (보호자 경고 마커 포함)
...
| `useLeafletMap` | 지도 인스턴스 재사용 (깜빡임 없음), 보호자 경고 마커 포함 |

### 백엔드 AI 구성

| 역할 | 모델 | 키 | 캐시 |
|------|------|----|------|
| 안전 브리핑 카드 | Claude Haiku (`claude-haiku-4-5-20251001`) | `ANTHROPIC_API_KEY` | 1시간 Django cache |
| AI 도우미 채팅 | Gemini 2.5 Flash Lite (`gemini-2.5-flash-lite`) | `GEMINI_API_KEY` | 없음 (멀티턴) |
| 개인화 안전 조언 | Gemini 2.5 Flash Lite (`gemini-2.5-flash-lite`) | `GEMINI_API_KEY` | 없음 |
...
| 보호자 뷰 | position:fixed 풀스크린 (헤더 50px + 맵 flex-grow + 하단 시트) |
...
| 풍속 ≥ 5m/s | -15 |
| 기온 ≤ 0°C 또는 ≥ 32°C | -20 |
| 비+강풍 동시 (≥5mm + ≥5m/s) | -20 추가 |
| 영하+강수 결빙 | -15 추가 |
| 불쾌지수(DI) ≥ 80 (열사병 위험) | -12 |
| 불쾌지수(DI) ≥ 75 | -6 |
| PM2.5 ≥ 75μg/m³ (매우나쁨) | -18 |
| PM2.5 ≥ 35μg/m³ (나쁨) | -9 |
| PM2.5 ≥ 15μg/m³ (보통) | -4 |
...
코스 > 가용시간:
  초과율 = (코스시간 - 가용시간) / 가용시간
  점수 = max(0.0, 0.7 - 초과율 × 1.4)

코스 ≤ 가용시간:
  활용률 = 코스시간 / 가용시간
  활용률 ≥ 65% → 1.0
  활용률 35~65% → 0.70~1.0 선형 보간
  활용률 < 35%  → max(0.30, 활용률/35% × 0.70)
```

**③ 난이도 적합도**

```
경험 매핑: beginner=1 / intermediate=2 / advanced=3
코스 난이도: easy=1 / medium=2 / hard=3

gap = |코스난이도 - 경험레벨|
점수 = max(0.0, 1.0 - gap × 0.45)
  gap 0 → 1.00  /  gap 1 → 0.55  /  gap 2 → 0.10
```

**④ 접근성**

```
max_km 이내: 1.0 - (거리 / max_km) × 0.30  (범위 0.70~1.0)
max_km 초과: max(0.0, 0.70 - 초과율 × 0.70)
```
...
raw × 데이터 품질 배수:
  거리 < 0.3km → 0.50  /  좌표 없음 → 0.75  /  일반 명칭 → 0.82  /  정상 → 1.0
...
**날씨** (`_weather_score`, 0.0~1.0): 코스 날씨 점수와 동일 방식.

**소요시간 적합도** (`_duration_score`, 0.0~1.0):
- 사용자 희망시간이 산의 최소~최대 소요 구간 내 → 1.0
- 희망 < 최소: `max(0.0, 1.0 - (최소-희망)/120)`
- 희망 > 최대: `max(0.0, 1.0 - (희망-최대)/180)`

**일몰 여유** (`_sunset_score`): 코스와 동일 기준 (90분→1.0 계단)

**이동 거리** (`_distance_score`, 0.0~1.0):

| 거리 | 기본 점수 |
|------|----------|
| ≤ 30km | 1.0 |
| 30~80km | 1.0 → 0.85 선형 |
| 80~150km | 0.85 → 0.65 선형 |
| 150~300km | 0.65 → 0.40 선형 |
| > 300km | 최저 0.20 |
...
6. 30분 위치 미갱신 → 빨간 pulsing 마커 + 경고 배너
7. 60분 위치 미갱신 → 긴급 팝업 (119 신고 유도)
```
...
```
guardian-shell (position: fixed; inset: 0)
├── guardian-header (50px)   ← 나가기 · 로고 · 상태 칩
├── guardian-stale-banner    ← 30분 경과 시만 표시
├── guardian-map-wrap (flex-grow)
│   ├── guardian-map-leaflet   (Leaflet 전용, 항상 빈 div)
│   └── guardian-map-overlay   (세션 없을 때만 표시)
└── guardian-sheet
    ├── sim-progress-bar
    ├── gbs-info-row (코스명 · 산 이름 · 상태 배지)
    ├── gbs-meta (마지막 수신 · 기록 횟수 · 소요 시간)
    ├── guardian-refresh-status / guardian-sim-status
    ├── gbs-sim-row (시뮬레이션 시작 버튼)
    └── gbs-actions (119 신고 · 새로고침/종료)
```
...
| 경과 시간 | 표시 내용 |
|----------|----------|
| 0 ~ 29분 | 파란 circleMarker (정상) |
| 30분 이상 | 빨간 pulsing divIcon + "⚠ N분 미갱신" 라벨 + 자동 팝업 열림 |
| 30분 이상 | 상단 빨간 경고 배너 (전화·119 유도) |
| 60분 이상 | "위치 미갱신 60분" 긴급 모달 (시뮬레이션 자동 종료) |

### 북한산 산행 시뮬레이션

🎬 버튼을 누르면 발표·데모용 시뮬레이션이 시작된다.

| 항목 | 내용 |
|------|------|
| 출발지 | 도선사 |
| 경유지 | 계곡 구간 → 지장암 갈림길 → 능선 합류 |
| 도착지 | 하루재 (중간 지점) |
| 이동 간격 | 1.6초/체크포인트 (실제 산행 5분 단위 표현) |
| 정지 후 시뮬레이션 | 틱당 5분 경과, 총 최대 70분 |
| 30분 경과 | 빨간 경고 마커 표시 + 배너 |
| 60분 경과 | 긴급 팝업 → 확인 시 초기화 |
...
### 커뮤니티 샘플 데이터 (Railway 적재 완료)

```bash
# 로컬에서 Railway DB로 fixture 로드
DATABASE_URL="<공개URL>" DJANGO_SECRET_KEY="..." \
  python manage.py loaddata recommendations/fixtures/sample_community.json
