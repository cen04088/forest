<template>
  <main class="guardian-shell">
    ...
    <!-- 풀스크린 맵 -->
    <div v-if="displaySession" ref="guardianMapEl" class="guardian-map" aria-label="산행자 현재 위치 지도"></div>
    <div v-else-if="!loading" class="guardian-map guardian-map-empty">
      <div class="guardian-empty-content">
        <p class="guardian-empty-msg">세이프 링크를 찾을 수 없습니다.<br>산행자에게 링크를 다시 받으세요.</p>
      </div>
    </div>
    <div v-else class="guardian-map guardian-map-loading">
      <div class="guardian-loading-spinner"></div>
      <p>위치 정보를 불러오는 중…</p>
    </div>

    <!-- 상단 헤더 오버레이 -->
    <header class="guardian-header-overlay">
    ...
    <!-- 하단 정보 시트 -->
    <div v-if="displaySession" class="guardian-sheet">
      ...
      <!-- 자동 새로고침 상태 (실제 세션) -->
      <div v-if="!simActive" class="guardian-refresh-status">
      ...
      <!-- 시뮬레이션 단계 표시 -->
      <div v-else class="guardian-sim-status">
        <span class="sim-dot"></span>
        <span>시뮬레이션 진행 중 — 체크포인트 {{ simStep }}/{{ SIM_WAYPOINTS.length }}</span>
        <span v-if="simStep >= SIM_WAYPOINTS.length" class="sim-stuck-label">정상 대기 중…</span>
      </div>
      ...
      <div class="gbs-actions">
        ...
        <button v-else class="outline-btn" type="button" @click="manualRefresh">새로고침</button>
      </div>

    </div>

    <!-- 시뮬레이션 시작 배너 (맵 없거나 실제 세션일 때 -->
    <div v-if="!simActive" class="guardian-sim-banner">
      <button class="sim-start-btn" type="button" @click="startSim">
        🎬 북한산 산행 시뮬레이션
      </button>
      <p class="sim-start-desc">보호자 화면이 어떻게 동작하는지 체험해보세요</p>
    </div>
...
const simStep = ref(0);       // 현재 도달한 웨이포인트 인덱스
const simStuckTicks = ref(0); // 정상에서 머문 틱 수
const simTrail = ref([]);
let _simInterval = null;

const simProgressPct = computed(() => {
  const total = SIM_WAYPOINTS.length + 4; // 도착 후 4틱 더
  const cur = simStep.value + simStuckTicks.value;
...
function _buildSimSession(trail, stuckSecs = 0) {
  ...
  return {
    ...
    safety_label: stuckSecs >= 660 ? '위치 미갱신' : '정상 이동',
    status: 'active',
...
function _doSimTick() {
  if (simStep.value < SIM_WAYPOINTS.length) {
    ...
    simStep.value++;
    const sess = _buildSimSession(simTrail.value, 0);
    if (guardianMapEl.value) renderGuardianMap(guardianMapEl.value, sess);
  } else {
    // 정상 도착 후 — 제자리에서 시간 경과 시뮬레이션
    simStuckTicks.value++;
    const stuckSecs = simStuckTicks.value * 180; // 틱당 3분 경과
    const sess = _buildSimSession(simTrail.value, stuckSecs);
    if (guardianMapEl.value) renderGuardianMap(guardianMapEl.value, sess);

    if (simStuckTicks.value >= 8) stopSim(); // 8틱(~13분) 후 자동 종료
  }
}
...
const displayStatusClass = computed(() => {
  if (simActive.value) {
    return simStuckTicks.value * 180 >= 660 ? 'yellow' : 'green';
  }
...
const showStaleWarning = computed(() => {
  if (simActive.value) return simStuckTicks.value * 180 >= 660; // 11분+
  return isLocationStale.value && session.value?.status !== 'ended';
});

const staleMinutes = computed(() => {
  if (simActive.value) return Math.floor(simStuckTicks.value * 3);
  return locationStaleMins.value;
});

// ── 위치 미수신 브라우저 알림 (실제 세션) ────────────────────────────────────
  ...
  if ('Notification' in window) {
    ...
    if (perm === 'granted') {
      new Notification('올라 — 위치 업데이트 없음', {
        body: `산행자의 위치가 ${locationStaleMins.value}분째 갱신되지 않았습니다.`,
        icon: '/logo.png',
      });
    }
  }
